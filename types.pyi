...
from ctypes import Array, Structure, c_double, c_long, c_ushort, c_void_p, sizeof
...
def ctype_for_type(tp: type[Any]) -> type[Any] | None:
...
def register_ctype_for_type(tp: type[Any], ctype: type[Any]) -> None:
    """Register a conversion from a Python type to a C type."""
    ...

def unregister_ctype_for_type(tp: type[Any]) -> None:
    """Unregister a conversion from a Python type to a C type."""
    ...

def get_ctype_for_type_map() -> dict[type[Any], type[Any] | None]:
...
_TStructure = TypeVar("_TStructure", bound=Structure)
_TArray = TypeVar("_TArray", bound=Array[Any])

@overload
def compound_value_for_sequence(
    seq: Iterable[Any], tp: type[_TStructure]
) -> _TStructure: ...
@overload
def compound_value_for_sequence(seq: Iterable[Any], tp: type[_TArray]) -> _TArray:
...
def CGSizeMake(w: float, h: float) -> CGSize: ...
def NSMakeRect(x: float, y: float, w: float, h: float) -> NSRect: ...
def CGRectMake(x: float, y: float, w: float, h: float) -> CGRect: ...
